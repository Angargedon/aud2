...
{
    // node->next->prev = node->prev; <- dit macht doch remove_at oder insert_at. müssen wir hier also nicht machen. 
...
void *dlist_get_data_at(dlist_t *list, int index)
{
    if (list != NULL)
    {
        dnode_t *get = dlist_get_node_at(list, index);
        return get->data;
    }
    else
    {
...
void dlist_insert_at(dlist_t *list, int index, void *data)
{
    ...
    if (list->first != NULL)
    {
        dnode_t *tmp = dlist_get_node_at(list, index);
        if (tmp == list->first)
            list->first = data;
...
void dlist_remove_at(dlist_t *list, int index){
    ...
    if (index > dlist_length(list))
    {
        ...
        dnode_delete(list->last->next);
        return;
    }
    dnode_t *rm = dlist_get_node_at(list, index);
    dnode_delete(rm);
}
