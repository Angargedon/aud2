...
void save(){
    ...
    if(file){
        if(Teamcounter > 0){
            fprintf(file, "<Data>\n");
            for(i = 0; i < Teamcounter; i++) saveTeam(file, &(Teams[i]));
            fprintf(file, "</Data>");
        }
        fclose(file);
    }
    ...
    do{
        ...
        if(strncmp(startRow, "<Name>", 6) == 0){
            ...
            if(strncmp(startRow + 6 + len, "</Name>", 7) == 0){
                ...
                if(Teams[Teamcounter].teamName) strncpy(Teams[Teamcounter].teamName, startRow+6, len);
                else{
        ...
        if(strncmp(startRow, "<Trainer>", 9) == 0){
            ...
            if(strncmp(startRow + 9 + len, "</Trainer>", 10) == 0){
                ...
                if(Teams[Teamcounter].trainerName) strncpy(Teams[Teamcounter].trainerName, startRow+9, len);
            }
        }
        if((strncmp(startRow, "<Player>", 8) == 0)){
            if(Teams[Teamcounter].playerAmount <= MAXPLAYER){
                if(loadPlayer(loadFile)) Teams[Teamcounter].playerAmount++;
                else printf("Teamindex %i Spielerindex %i: Laden fehlgeschagen.", Teamcounter, Teams[Teamcounter].playerAmount);
            }
...
void load(){
    ...
    if(file){
        ...
        printf("\nDatei konnte ich aufgemacht werden.");
    }
}